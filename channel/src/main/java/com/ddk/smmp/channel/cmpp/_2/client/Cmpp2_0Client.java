					...
					try {
						...
						connector.addListener(new IoListener(){
							...
							public void sessionDestroyed(IoSession session) throws Exception {
								ChannelLog.log(logger, "[cmpp2.0]监听到session关闭", LevelUtils.getSucLevel(channel.getId()));
								
								for (;;) {
									if(channel.isReConnect()){
										ChannelLog.log(logger, "[cmpp2.0]开始重连", LevelUtils.getSucLevel(channel.getId()));
										...
										try {
											...
											ChannelLog.log(logger, "[cmpp2.0]开始重连,强制关闭之前session" + e.getMessage(), LevelUtils.getSucLevel(channel.getId()), e.getCause());
										}
						                ...
						                try {
						                    ...
						                    if (session.isConnected() && null != session.getAttribute("isSend")) {
						                    	...
						                    	ChannelLog.log(logger, "[cmpp2.0]断线重连[" + defaultAddress.getHostName() + ":" + defaultAddress.getPort() + "]成功", LevelUtils.getSucLevel(channel.getId()));
						                        
						                    	break;
						                    }
						                } catch (Exception ex) {
						                	ChannelLog.log(logger, "[cmpp2.0]重连服务器登录失败,60秒再连接一次:" + ex.getMessage(), LevelUtils.getSucLevel(channel.getId()), ex.getCause());
						                }
									}else{
										break;
									}
