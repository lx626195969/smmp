...
public class SubmitThread extends Thread{
			...
			public void run() {
				...
				if(limit > 0 && null != channel.getSession() && channel.getSession().isConnected() && null != channel.getSession().getAttribute("isSend")){
					...
					try {
						...
						ChannelLog.log(logger, ex.getMessage(), LevelUtils.getErrLevel(channel.getId()), ex);
						trans.rollback();
					} finally {
						trans.close();
					}
	...
	private void startSubmitData() {
		List<SmQueue> tempList = null;
		while(isContinue){
			tempList = new LinkedList<SmQueue>();
			...
			try {
				...
				ChannelLog.log(logger, e1.getMessage(), LevelUtils.getErrLevel(channel.getId()), e1);
			}
		}
	}
