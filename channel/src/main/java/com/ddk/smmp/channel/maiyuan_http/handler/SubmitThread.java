...
public class SubmitThread extends Thread{
	...
	public SubmitThread(Channel channel) {
		...
		if(channel.getIsBatch().intValue() == 0){
			this.submitDataThreadPool = Executors.newFixedThreadPool(2);
		}else{
			this.submitDataThreadPool = Executors.newFixedThreadPool(1);
		}
	...
	public void run() {
		if(channel.getIsBatch().intValue() == 0){
			...
			startWatchData_batch();
			startSubmitData_batch();
		}
	}
	 ...
	 * 从数据库获取待发送数据-批量
	 */
	private void startWatchData_batch() {
		watchDataThreadPool.scheduleAtFixedRate(new Runnable() {
			@Override
			public void run() {
				//数据库查询出来并锁定的数据
				...
				if(queue.size() == 0){
					DatabaseTransaction trans = new DatabaseTransaction(true);
					try {
						temp = new DbService(trans).getMsgFromQueueAndLockMsg_batch(channel.getId(), smgFlowLimit);
						trans.commit();
					} catch (Exception ex) {
						ChannelLog.log(Logger.getLogger(SubmitThread.class), ex.getMessage(), LevelUtils.getErrLevel(channel.getId()));
						trans.rollback();
	 ...
	 * 将数据提交到运营商
	 */
	private void startSubmitData_batch() {
		while(isContinue){
			List<SmQueue> tempList = new LinkedList<SmQueue>();
			...
			try {
				...
				if(queue.size() > 0){
					queue.retainAll(tempList);
					
					submitDataThreadPool.execute(new SubmitChildBatchThread(tempList, channel));
				}else{
					Thread.sleep(1000);
				}
			} catch (InterruptedException e1) {
