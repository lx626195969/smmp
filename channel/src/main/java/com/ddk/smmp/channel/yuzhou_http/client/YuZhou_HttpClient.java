	...
	public void start() {
		...
		if(null == yuZhou_HttpServer){
			...
			try {
				...
				ChannelLog.log(logger, "启动宇宙之讯报告和上行处理线程失败......" + e.getMessage(), LevelUtils.getErrLevel(channel.getId()));
	...
	public void stop() {
		...
		if(null != yuZhou_HttpServer){
			try {
				...
				ChannelLog.log(logger, "停止宇宙之讯短信提交处理线程......" + e.getMessage(), LevelUtils.getErrLevel(channel.getId()));
			}
		}
